...
\subsubsection{A modell, a bemenet és a címke}
...
\subsubsection{Targeted és non-targeted mód}
...
\subsubsection{A white-box hozzáférési feltétel}
...
\subsubsection{A szoftver célja és beállítható paraméterei}
...
lesz a későbbiekben bemutatott adversarial-példa-generáló algoritmusok leírásában. 

\subsubsection{Perturbáció és normakorlát}

Az adversarial képek definíciójánál szokás a perturbáció nagyságát valamely $\ell_p$ normával jellemezni; ebben a dolgozatban a $\ell_\infty$ normát használjuk. 
...
Tehát az FGSM adversarial példát a következő módon kapjuk meg: 
\[
...
A célzott változatnál egy $y_{\mathrm{target}}$ célosztályt adunk meg, és azt szeretnénk, hogy a modell ebbe az osztályba sorolja az adversarial képet. 
\[
...
Ezt az módszert a Mandry és társai vezették be és lényegében az FGSM egylépéses ötletének iteratív változata. 
Tehát az FGSM-et alkalmazva kapott adversarial képet alapul véve újra végrehajtjuk. 
A motiváció emögött az, hogy az FGSM linearitási feltevés csak x kis környezetében állja meg a helyét, ezért előnyös ha T darab kisebb alfa lépéssel 
dolgozunk és a gradienst minden alkalommal újraszámoljuk. 
Mivel ez az eljrás túlléphet az epsilon sugarú l_p értelemben vett gömb korláton, az algoritmus minden lépés után visszavágja egy projekcióval a gömbre. 
Ez az FGSM-nél a sign függvény maitt adaott volt, itt T és alfa megválasztásától függően szükséges lehet. 
így az algoritmus formális definíciója 
...
∞
​
 ≤ϵ} az engedélyezett perturbációk halmaza, 
Π
x
+
S
...
x+S
​
  pedig az erre a halmazra való projekció (gyakorlatban egyszerű clamp/vágás).

\subsubsection{Célzott változat}
FGSM-hez hasonlóan a itt is definiálunk egy célzott változatot, ami az ottanival analóg módon definiálható. 
...
\subsubsection{T és alfa megválasztása}
itt vezetjük be természetes módon 
α
α-t (lépésnagyság) és 
T
T-t (iterációszám) mint a formulában szereplő paramétereket, rögtön jelezve a szerepüket: nagyobb 
T
T általában erősebb támadást ad, de több gradiensszámítást igényel; 
α
α-t úgy kell megválasztani, hogy 
T
T lépés alatt be tudja járni az 
ϵ
ϵ-korlátozta teret (tipikusan 
α
≈
ϵ
/
...
\subsubsection{Véletlen kiindulási pont}
Madry et al. kísérletileg mutatták, hogy a veszteségfelszín lokális maximumai jól koncentráltak, így egy véletlen kezdőpontból indított PGD is 
megbízhatóan jó megoldást talál. 
Ennek a szerepe a modellek robosztusságának a tesztelése, mivel a veszteségfelszínnek több egymástól távolabb eső lokális maximuma is lehet és itt a 
kezdőponttól függően a szerencsének nagy szerepe lehet és nem feltétlenül mutatja a modell valódi sebezhetőségét. 
Ebben az esetben a véletlen kezdőpontot az epsilon sugarú gömbön egyenletes eloszlással választjuk meg. 
...
\subsubsection{Alpha és T megválasztása}

A PGD viselkedését három fő paraméter határozza meg. Ezek közül epsilont ismerjük, mint a perturbáció felső korlátját. 
T az iteráció számát jelöli, tehát az algoritmus számítási költségét adja meg, de egyben erősebb támadást eredményez. 
Az alpha megválasztása olyan kell legyen, hogy ha túl kicsi, akkor lassú a konvergencia, ám ha túl nagy, akkor instabilitáshoz vezethet. 
...
A random restart technikával kombinálva különösen hatékony. 
