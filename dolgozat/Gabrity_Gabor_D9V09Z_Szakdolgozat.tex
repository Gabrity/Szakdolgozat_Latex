...
\subsection{A probléma részletes specifikációja}  %(a fejezet elejét egy kicsit bővítsd ki, hogy ne egyből a Legyen adott... mélybe ugrás előtt)
...
\subsubsection{Az osztályozó modell és a logitok}
...
Ezt logit vektornak is nevezzük, mely elnevezés a logisztikus regresszióból származik, ahol a $\mathrm{logit}(p) = \ln\frac{p}{1-p}$ függvény egy valószínűséget 
...
\subsubsection{Valószínűségek és a modell predikciója}
...
\subsubsection{Veszteségfüggvény és bemeneti gradiens}
...
A gradiens a súlyokra nézve, $\nabla_\theta J(\theta, x, y)$, az a gradiensvektor, amelyet a backpropagation során a tanításban használunk a súlyok módosítására. 
Itt most a gradiensszámítást egy további lépéssel a legutolsó, bemeneti rétegig visszük, és megkapjuk a bemeneti vektorra vonatkozó érzékenységet, $\nabla_x J(\theta, x, y)$. 
Ez a mennyiség azt mutatja meg, hogy egy adott bemeneti pixel vagy jellemző változása milyen mértékben befolyásolja a veszteséget, ezért központi szerepe 
lesz a későbbiekben az adversarial példák generálásának és az algoritmusok leírásának. 

\subsubsection{Perturbáció és normakorlát}

Pár mondat az adversarial képek definiálásánál használt $L_p$ normákról.   
...
A perturbáció nagyságát egy norma szerint korlátozzuk:  %(ez rossz, L_inf normát akartam írni, illetve hiányzik a definíciója is)
\[
  \|\delta\|_p \leq \epsilon.
\]
Ez esetünkben nagyon kézenfekvő, mivel a bemeneti kép minden pixelén a változást külön-külön korlátozza. 

\subsection{Ellenfél alapú támadások áttekintése}
A mély neurális hálók, köztük a képfelismerő modellek különösen érzékenyek a bemeneti adatok kicsi ám célzott módosításaira. %(hivatkozást add hozzá)
...
Az FGSM algoritmust Goodfellow és társai javasolták a google-nél dolgozva. %(hivatkozás)
Az előző fejezetben megjegyeztem, hogy a $\nabla_x J(\theta, x, y)$ célfüggvény gradiense az input értékre való érzékenységet jelenti. 
Adja magát, hogy léptessük el a gradienssel ellenkező irányba az inputot, ami várhatóan rontani fogja a célfüggvény értékét. 
Ehhez azt feltételezzük, hogy a modell az x kellően kis környezetében lineárisként viselkedik. %(ez miért lehet, lehetne erről egy külön fejezet?)
Az FGSM algoritmusban az elmozdítást egyszerűen csak a sign előjel-függvény epszilonszorosával tesszük meg, tehát nem vesszük figyelembe a gradiens által adott érzékenységet, csak az irányt. 
Viszont ilyen módon az epsilon x sign maximálizája az epsilon költsévgvetésünket minden xi-re. 
Tehát az FGSM adversarial képet e módon kapjuk meg: 
x adv =x+ϵ⋅sign ∇xJ (θ, x, y)
...
A célzott változatnál ytarget célosztályt adunk meg és azt szeretnénk, hogy a modell ebbe az osztályba sorolja az adversarial képet. 
xadv = x − ϵ · sign ∇xJ (θ, x, ytarget)
Tehát a képlet csak annyiban változik, hogy távolodás helyett közeledni szeretnénk és y helyett ytarget-hoz. 
...
Hátránya, hogy csak egy durva lépést tesz a megfelelő irányba, így adódhat, hogy kicsi változást érünk el vele az osztályozásban, 
például az iteratív módszerekhez képest. 
